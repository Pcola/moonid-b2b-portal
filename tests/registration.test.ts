...
// Overuje: validáciu, honeypot (bot ticho odmietnutý), povinný GDPR súhlas, rate-limit a zápis
...
vi.mock("@/lib/rate-limit", () => ({ rateLimit: vi.fn(), clientIp: vi.fn().mockReturnValue("1.2.3.4") }));
...
const base = { ico: "12345678", companyName: "Reg Test s.r.o.", contactName: "Ján Test", email: EMAIL, gdpr: true };
...
describe("createAccessRequest — žiadosť o prístup", () => {
  it("platný vstup + GDPR súhlas → založí žiadosť (PENDING)", async () => {
  ...
  it("bez GDPR súhlasu → ok:false, žiadny zápis", async () => {
    const res = await createAccessRequest({ ...base, gdpr: false });
    expect(res.ok).toBe(false);
    expect(res.error).toMatch(/súhlas/i);
    expect(await prisma.accessRequest.count({ where: { email: EMAIL } })).toBe(0);
  });
