...
// Kolegovia v rovnakej firme si navzájom nemenia položky; companyId ostáva defense-in-depth.
...
export async function getOrCreateCart(companyId: string, userId: string) {
  try {
    ...
    if (cart.companyId !== companyId) throw new Error("Košík používateľa patrí inej firme.");
    return { id: cart.id };
  } catch (e) {
    if (e instanceof Prisma.PrismaClientKnownRequestError && e.code === "P2002") {
      const existing = await prisma.cart.findFirst({ where: { createdById: userId, companyId }, select: { id: true } });
...
export async function getCartCount(companyId: string, userId: string): Promise<number> {
  const cart = await prisma.cart.findFirst({ where: { companyId, createdById: userId }, select: { id: true } });
  if (!cart) return 0;
...
export async function getCartDetail(companyId: string, userId: string, tierCode: string | null, discountPct: number): Promise<CartDetail> {
  const cart = await prisma.cart.findFirst({ where: { companyId, createdById: userId }, select: { id: true } });
