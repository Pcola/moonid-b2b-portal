...
export function FavoriteButton({ productId, initial, className = "" }: { productId: string; initial: boolean; className?: string }) {
  ...
  // #e0a83b malo 2,14:1 na bielej — grafický indikátor stavu potrebuje 3:1 (SC 1.4.11); #a8761a = 3,98:1
  return (
    <button
      type="button"
      onClick={toggle}
      ...
      className={`flex h-8 w-8 flex-none items-center justify-center rounded-full transition disabled:opacity-50 ${fav ? "text-[#a8761a]" : "text-muted-2 hover:text-ink"} ${className}`}
    >
