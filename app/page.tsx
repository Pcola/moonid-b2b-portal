...
const CATEGORIES = [
  { t: "Čistiace a dezinfekčné prostriedky", d: "Univerzálne čističe, dezinfekcia povrchov a rúk, kúpeľňa a WC, čističe na rúry a grily, profesionálna chémia." },
  { t: "Papierový program", d: "Toaletný papier, papierové utierky (skladané aj rolované), uteráky, servítky, vreckovky." },
  { t: "Dávkovače a zásobníky", d: "Systémy na mydlo a penu, toaletný papier, utierky, dezinfekciu, sušiče rúk aj osviežovače vzduchu." },
...
export default function Home() {
  return (
    <>
      ...
      <main id="top">
        {/* ===== HERO ===== */}
        <section className="relative flex items-end overflow-hidden" style={{ minHeight: "clamp(580px,82vh,860px)" }}>
        ...
        {/* ===== SORTIMENT ===== */}
        <section id="sortiment" className="bg-cream" style={sectionPad}>
          <div className={wrap}>
              ...
              {CATEGORIES.map((c) => (
                <div key={c.t} className="card-hover flex min-h-[172px] flex-col gap-3 rounded-[14px] border border-line bg-white p-[clamp(22px,2.5vw,28px)]">
                  <div className="flex justify-end">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#1e5249" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14M13 6l6 6-6 6" /></svg>
                  </div>
              ...
              {[
                ...
                <div key={c.t} className="flex flex-col overflow-hidden rounded-[18px] border border-line bg-white">
                  <div className="relative h-[340px] w-full">
                    <Image src={c.img} alt={c.t} fill sizes="(max-width:1024px) 100vw, 50vw" className="object-cover" />
                  </div>
                  <div className="flex flex-col gap-3 p-[clamp(26px,3vw,36px)]">
                    <h3 className="text-ink" style={{ fontSize: "clamp(22px,2.4vw,28px)", letterSpacing: "-0.01em" }}>{c.t}</h3>
        ...
        {/* ===== PORTÁL ===== */}
        <section id="portal" className="bg-white" style={sectionPad}>
          <div className={wrap}>
            <div className="reveal relative overflow-hidden rounded-[24px] text-mintbg" style={{ background: "radial-gradient(130% 130% at 88% -20%, #21564C 0%, #163F38 56%)" }}>
              ...
              <div className="relative z-[2] grid items-center gap-[clamp(36px,5vw,72px)] p-[clamp(34px,5vw,72px)] lg:grid-cols-[1.15fr_0.85fr]">
                ...
                <div className="flex flex-col gap-4 rounded-[18px] border border-white/15 bg-white/[0.06] p-[clamp(26px,3vw,36px)]">
                  ...
                  <Link href="/login" className="inline-flex items-center justify-center gap-2.5 rounded-[10px] bg-white px-[22px] py-3.5 text-[15.5px] font-semibold text-brand transition hover:-translate-y-px hover:bg-mintbg">
        ...
        <section id="preco" className="relative overflow-hidden text-mintbg" style={{ ...sectionPad, background: "radial-gradient(130% 120% at 85% -10%, #21564C 0%, #163F38 52%)" }}>
          <div className={wrap}>
            <div className="reveal mb-[clamp(40px,5vw,64px)] max-w-[640px]">
              ...
              <h2 className="mt-3.5 text-white" style={{ ...h2style, textWrap: "balance" }}>Jeden spoľahlivý partner pre celý chod prevádzky</h2>
            </div>
            <div className="reveal-cascade grid gap-[clamp(18px,2vw,24px)]" style={{ gridTemplateColumns: "repeat(auto-fit,minmax(248px,1fr))" }}>
              {WHY.map((w) => (
                <div key={w.t} className="why-card flex flex-col gap-[18px] rounded-[18px] border border-white/10 bg-white/[0.045] p-[clamp(26px,2.6vw,32px)]">
                  <span className="inline-flex h-[54px] w-[54px] items-center justify-center rounded-[14px] border border-[#8fc3b9]/25 bg-[#8fc3b9]/[0.13] text-[#a9d8cd]">
                    <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">{w.p}</svg>
                  </span>
                  <div className="flex flex-col gap-2.5">
                    <h3 className="text-xl font-semibold text-white" style={{ letterSpacing: "-0.01em" }}>{w.t}</h3>
        ...
        {/* ===== KONTAKT ===== */}
        <section id="kontakt" className="relative overflow-hidden bg-brand-deep text-[#e7efec]" style={{ padding: "clamp(72px,10vw,132px) 0" }}>
            ...
            <div className="grid items-start gap-[clamp(44px,6vw,96px)] lg:grid-cols-[0.9fr_1.1fr]">
              <div className="reveal flex flex-col">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-[11.5px] font-semibold uppercase tracking-[0.22em] text-[#8fb3ab]">Kontakt</span>
                </div>
                <h2 className="mt-[22px] text-white" style={{ fontSize: "clamp(32px,4.4vw,54px)", lineHeight: 1.04, letterSpacing: "-0.02em", textWrap: "balance" }}>Začnime spoluprácu</h2>
                <p className="mt-[18px] max-w-[430px] text-[#a9c2bb]" style={{ fontSize: "clamp(16px,1.6vw,18.5px)", lineHeight: 1.6 }}>Napíšte nám, čo vaša prevádzka potrebuje — pripravíme cenovú ponuku na mieru a ozveme sa vám spravidla do 24 hodín.</p>
                ...
                <div className="mt-[clamp(36px,4vw,52px)] flex flex-col gap-1">
                  ...
                  <a href="tel:+421919216908" className="font-medium text-white transition hover:text-[#9fe0cf]" style={{ fontSize: "clamp(26px,3vw,34px)", letterSpacing: "-0.01em" }}>0919 216 908</a>
                </div>
                <div className="mt-6 flex flex-col gap-1">
                  <span className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[#6e938b]">E-mail</span>
                  <a href="mailto:moonid@moonid.sk" className="font-medium text-[#e7efec] transition hover:text-[#9fe0cf]" style={{ fontSize: "clamp(18px,1.8vw,21px)" }}>moonid@moonid.sk</a>
                </div>
              ...
              <div className="reveal rounded-[14px] p-[clamp(28px,3.4vw,46px)]" style={{ background: "#fbfcfb", boxShadow: "0 40px 80px -40px rgba(0,0,0,0.55)" }}>
                <ContactForm />
              </div>
            </div>
          </div>
