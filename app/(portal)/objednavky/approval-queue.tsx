...
export function ApprovalQueue({ orders }: { orders: QOrder[] }) {
  ...
  return (
    <div className="mb-6 rounded-2xl border border-[#e8d9a8] bg-[#fdf9ee] p-5">
      <h2 className="text-[16px] font-semibold text-warning-ink">Čaká na vaše schválenie ({orders.length})</h2>
        ...
        {orders.map((o) => (
          <div key={o.id} className="flex flex-wrap items-center gap-x-4 gap-y-2 rounded-xl border border-line bg-white px-4 py-3">
            ...
            <div className="flex flex-none items-center gap-2">
              <button onClick={() => act(o.id, rejectOrder)} disabled={pending} className="rounded-[9px] border border-line px-3 py-2 text-[13px] font-semibold text-danger-ink transition hover:border-[#e0b0a8] disabled:opacity-50">{busyId === o.id ? "…" : "Zamietnuť"}</button>
      ...
      {err && <p className="mt-2 text-[13px] text-danger-ink">{err}</p>}
    </div>
  );
}
