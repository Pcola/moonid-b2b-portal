...
export default async function StaffCustomers() {
  ...
  const [companies, revRows, tiersCount] = await Promise.all([
    prisma.company.findMany({
      ...
      select: { id: true, name: true, city: true, priceTier: { select: { code: true, discountPct: true } }, users: { take: 1, orderBy: { createdAt: "asc" }, select: { email: true } }, _count: { select: { orders: true } } },
    }),
  ...
  return (
    <div className="flex max-w-[1240px] flex-col gap-5">
      <p className="text-[14px] text-muted">{companies.length} aktívnych zákazníkov · {tiersCount} cenové úrovne</p>
      <div className="overflow-hidden rounded-2xl border border-line bg-white">
        ...
        {companies.length === 0 ? (
          ...
          return (
            <div key={c.id} className="grid grid-cols-[2fr_1fr_1fr_0.7fr_1fr] items-center gap-4 border-b border-line px-[22px] py-4 last:border-0">
