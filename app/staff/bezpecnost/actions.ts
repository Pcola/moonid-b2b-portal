...
export async function markMfa(enabled: boolean): Promise<{ ok: boolean }> {
  const user = await getCurrentUser();
  if (!user || (user.role !== "STAFF" && user.role !== "ADMIN")) return { ok: false };
  await prisma.user.update({ where: { id: user.id }, data: { mfaEnabled: enabled } }).catch(() => {});
  await writeAudit({ userId: user.id, action: enabled ? "MFA_ENROLLED" : "MFA_REMOVED", entity: "Auth", entityId: user.id });
  return { ok: true };
}
